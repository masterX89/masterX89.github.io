...
> 本章节转载自：[javascript 总结（RegExp篇）](https://segmentfault.com/a/1190000012699314)
>
> 作者：[**咚子**](https://segmentfault.com/u/zi_597d64ce14187)
>
> 发布时间：2018-01-04
...
1. compile(regexp,modifier): 在脚本执行过程中编译正则表达式
...
2. exec(string): 用于检索字符串中的正则表达式的匹配。如果字符串中有匹配的值返回该匹配值，否则返回 null。
...
3. test(string): 用于检测一个字符串是否匹配某个模式.如果字符串中有匹配的值返回 true ，否则返回 false
...
4. match(regexp): 可在字符串内检索指定的值，或找到一个或多个正则表达式的匹配。如果没有找到任何匹配的文本， match() 将返回 null。否则，它将返回一个数组，其中存放了与它找到的匹配文本有关的信息。
